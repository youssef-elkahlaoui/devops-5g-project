...
### Issue 8: WebUI Not Accessible Externally (ERR_CONNECTION_REFUSED)

**Symptom:** WebUI service is running but http://<EXTERNAL_IP>:9999 returns connection refused

**Root Cause:** WebUI binds to 127.0.0.1:9999 (localhost only) instead of 0.0.0.0:9999 (all interfaces)
...
# Check what interface WebUI is listening on
sudo ss -tlnp | grep 9999
# PROBLEM: Shows "127.0.0.1:9999" instead of "0.0.0.0:9999"

# Verify firewall (GCP)
gcloud compute firewall-rules describe allow-5g-lab
# Should show: sourceRanges: 0.0.0.0/0, IPProtocol: all
```

**Solution - Fix WebUI Binding:**

```bash
# Create fix script
cat > /tmp/fix-webui.sh << 'EOF'
#!/bin/bash
...
chmod +x /tmp/fix-webui.sh
sudo bash /tmp/fix-webui.sh
```

**Verification:**

```bash
# Confirm WebUI now listens on all interfaces
sudo ss -tlnp | grep 9999
# EXPECTED: "LISTEN 0 511 0.0.0.0:9999"

# Test from browser
# Open: http://<EXTERNAL_IP>:9999
# Login: admin / 1423
```

**Why This Happens:**

- Default Open5GS WebUI installation uses `const _hostname = process.env.HOSTNAME || 'localhost';`
- This binds to 127.0.0.1, blocking external access
- Must change to `'0.0.0.0'` to accept connections from any interface
- GCP firewall rules alone don't fix this - application must explicitly bind to 0.0.0.0

**For Ansible Automation:**

Add this task after WebUI installation in your playbook:
...
### Issue 9: Prometheus Port Conflict (Port 9090)

**Symptom:** Prometheus fails to start with "listen tcp 0.0.0.0:9090: bind: address already in use"

**Root Cause:** Open5GS services (AMF, SMF, UPF, PCF) expose metrics on port 9090
...
# Shows: open5gs-amfd, open5gs-smfd, etc.

# Check Prometheus status
sudo systemctl status prometheus
# Shows: "bind: address already in use"
```

**Solution - Change Prometheus to Port 9091:**

```bash
# Stop Prometheus
sudo systemctl stop prometheus

# Update Prometheus arguments
...
# EXPECTED: "prometheus" on port 9091
```
...
# Open: http://<EXTERNAL_IP>:9091
# Should show Prometheus UI

# Verify Open5GS metrics still available on 9090
...
```yaml
- name: Configure Prometheus to use port 9091 (avoid Open5GS conflict)
  copy:
    content: |
      ARGS="--web.listen-address=0.0.0.0:9091"
...
- **Port 9090:** Open5GS services metrics (AMF, SMF, UPF, PCF)
- **Port 9091:** Prometheus web interface
- **Port 9999:** Open5GS WebUI
...
- [ ] WebUI accessible at http://<EXTERNAL_IP>:9999 (listening on 0.0.0.0:9999)
- [ ] Prometheus accessible at http://<EXTERNAL_IP>:9091 (if installed)
